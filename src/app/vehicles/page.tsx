...
const EMPTY_VEHICLE_FORM: VehicleFormState = { id: null, name: "", number: "", departmentId: "", sortOrder: "0", isActive: true };

type DepartmentFormState = { id: number | null; name: string; sortOrder: string; isActive: boolean };
const EMPTY_DEPARTMENT_FORM: DepartmentFormState = { id: null, name: "", sortOrder: "0", isActive: true };

const NETWORK_ERROR_MESSAGE = "通信エラーが発生しました。電波・Wi-Fiの状態を確認して、もう一度お試しください。";
...
export default function VehiclesPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const [vData, dData]: [VehicleDTO[], DepartmentDTO[]] = await Promise.all([vRes.json(), dRes.json()]);
      setVehicles(vData);
  ...
  async function handleRevive(v: VehicleDTO) {
    ...
    try {
      const res = await fetch(`/api/vehicles/${v.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: v.name, number: v.number, departmentId: v.departmentId, sortOrder: v.sortOrder, isActive: true }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
  ...
  function startEditDept(d: DepartmentDTO) {
    setDeptForm({ id: d.id, name: d.name, sortOrder: String(d.sortOrder), isActive: d.isActive });
  ...
  async function handleDeptSubmit(e: FormEvent) {
    ...
    const payload = { name: deptForm.name.trim(), sortOrder: Number(deptForm.sortOrder) || 0, isActive: deptForm.isActive };
  ...
  async function handleDeptDeleteConfirmed() {
    ...
    try {
      const res = await fetch(`/api/departments/${deptDeleteTarget.id}`, { method: "DELETE" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
  ...
  async function handleDeptRevive(d: DepartmentDTO) {
    ...
    try {
      const res = await fetch(`/api/departments/${d.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: d.name, sortOrder: d.sortOrder, isActive: true }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
  ...
  const sortedVehicles = [...vehicles].sort((a, b) => a.sortOrder - b.sortOrder || a.id - b.id);
  ...
  const sortedDepartments = [...departments].sort((a, b) => a.sortOrder - b.sortOrder || a.id - b.id);
      ...
      <main className="flex-1 px-6 py-6">
        ...
        <form onSubmit={handleAuthSubmit} className="card max-w-sm mx-auto flex flex-col gap-5">
          <div>
            ...
            <p className="text-gray-500 text-sm">車両管理には管理者パスワードが必要です。</p>
          </div>
          {authError && (
            <p className="text-red-600 font-bold bg-red-50 border-2 border-red-300 rounded-lg px-4 py-2">{authError}</p>
          )}
          <input
            type="password"
            autoFocus
    ...
    <main className="flex-1 px-6 py-6 pb-16">
      ...
      <div className="mb-8">
        ...
        >
          ...
          <span className={`shrink-0 text-gray-400 transition-transform ${showDepartments ? "rotate-180" : ""}`} aria-hidden="true">
            ▼
          </span>
        </button>
        ...
        {showDepartments && (
          <div className="mt-3 flex flex-col gap-4">
            <form onSubmit={handleDeptSubmit} className="card flex flex-col gap-4">
              <h3 className="text-base font-bold text-gray-700">{deptForm.id ? "所属を編集" : "所属を新規登録"}</h3>
              {deptErrorMessage && (
                <p className="text-red-600 font-bold bg-red-50 border-2 border-red-300 rounded-lg px-4 py-2">{deptErrorMessage}</p>
              )}
              <div>
              ...
              <div className="flex gap-6">
                <div className="flex-1">
                  ...
                  <input type="number" className="input-field" value={deptForm.sortOrder} onChange={(e) => setDeptForm((f) => ({ ...f, sortOrder: e.target.value }))} />
                </div>
                <div className="flex-1">
                  <label className="label-text">状態</label>
                  <select className="input-field bg-white" value={deptForm.isActive ? "active" : "inactive"} onChange={(e) => setDeptForm((f) => ({ ...f, isActive: e.target.value === "active" }))}>
              ...
              <div className="flex gap-3">
                ...
                {deptForm.id && (
                  <button type="button" onClick={resetDeptForm} className="btn-secondary flex-1">
                    新規登録に戻る
                  </button>
                )}
              </div>
            ...
            {activeDepartments.length > 0 && (
              <div className="flex flex-col gap-2">
                {activeDepartments.map((d) => (
                  <div key={d.id} className="flex items-center justify-between gap-4 rounded-xl border border-gray-200 bg-white px-4 py-3">
                    <div className="font-bold text-gray-800">{d.name}</div>
                    <div className="flex gap-2 shrink-0">
                      <button onClick={() => startEditDept(d)} className="btn-small border-brand-500 text-brand-600">
                        編集
                      </button>
                      <button
                        onClick={() => {
            ...
            {inactiveDepartments.length > 0 && (
              <div>
                <h4 className="text-sm font-bold text-gray-500 mb-2">非表示の所属（使用停止中）</h4>
                {deptReviveError && <p className="text-red-600 font-bold mb-2 text-sm">{deptReviveError}</p>}
                <div className="flex flex-col gap-2">
                  {inactiveDepartments.map((d) => (
                    <div key={d.id} className="flex items-center justify-between gap-4 rounded-xl border border-gray-200 bg-gray-50 px-4 py-2">
                      <div className="text-gray-600 text-sm font-bold">{d.name}</div>
                      <div className="flex gap-2 shrink-0">
                        <button onClick={() => handleDeptRevive(d)} className="btn-small border-emerald-500 text-emerald-600">
                          復活させる
                        </button>
                        <button onClick={() => startEditDept(d)} className="btn-small border-brand-500 text-brand-600">
                          編集
      ...
      <form onSubmit={handleSubmit} className="card flex flex-col gap-5 mb-8">
        <h2 className="text-lg font-bold text-gray-700">{form.id ? "車両を編集" : "車両を新規登録"}</h2>
        {errorMessage && (
          <p className="text-red-600 font-bold bg-red-50 border-2 border-red-300 rounded-lg px-4 py-2">{errorMessage}</p>
        )}
        <div>
          <label className="label-text">車両名</label>
          <input type="text" className="input-field" placeholder="例：営業車1号" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} />
        </div>
        <div>
          <label className="label-text">車両番号</label>
          <input type="text" className="input-field" placeholder="例：品川300 あ 12-34" value={form.number} onChange={(e) => setForm((f) => ({ ...f, number: e.target.value }))} />
        </div>
        <div>
          <label className="label-text">所属</label>
          <select className="input-field bg-white" value={form.departmentId} onChange={(e) => setForm((f) => ({ ...f, departmentId: e.target.value }))}>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            <label className="label-text">状態</label>
            <select className="input-field bg-white" value={form.isActive ? "active" : "inactive"} onChange={(e) => setForm((f) => ({ ...f, isActive: e.target.value === "active" }))}>
              <option value="active">使用中</option>
              <option value="inactive">使用停止</option>
            </select>
          </div>
              ...
              <div className="flex flex-col gap-3">
                {g.vehicles.map((v) => (
                  <div key={v.id} className="card flex items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2 shrink-0">
                      <button onClick={() => startEdit(v)} className="btn-small border-brand-500 text-brand-600">
                        編集
                      </button>
                      <button
                        onClick={() => {
      ...
      {!loading && !loadError && inactiveVehicles.length > 0 && (
        <div className="mt-8">
          <h2 className="text-base font-bold text-gray-500 mb-3">非表示の車両（使用停止中）</h2>
          {reviveError && <p className="text-red-600 font-bold mb-3">{reviveError}</p>}
          <div className="flex flex-col gap-2">
            {inactiveVehicles.map((v) => (
              <div key={v.id} className="flex items-center justify-between gap-4 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3">
                <div>
                  <div className="font-bold text-gray-600 text-sm">{v.name}</div>
                  <div className="text-gray-400 text-xs">{v.number}</div>
                </div>
                <div className="flex gap-2 shrink-0">
                  <button onClick={() => handleRevive(v)} className="btn-small border-emerald-500 text-emerald-600">
                    復活させる
                  </button>
                  <button onClick={() => startEdit(v)} className="btn-small border-brand-500 text-brand-600">
                    編集
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center px-6 z-50">
          <div className="card max-w-sm w-full">
            <p className="text-lg font-bold text-gray-800 mb-2">「{deleteTarget.name}」を削除しますか？</p>
            <p className="text-gray-500 mb-4">利用記録がある場合は削除されず、代わりに非表示（使用停止）になります。</p>
      ...
      {deptDeleteTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center px-6 z-50">
          <div className="card max-w-sm w-full">
            <p className="text-lg font-bold text-gray-800 mb-2">「{deptDeleteTarget.name}」を削除しますか？</p>
            <p className="text-gray-500 mb-4">紐づく車両がある場合は削除されず、代わりに非表示（使用停止）になります。</p>
            {deptDeleteError && <p className="text-red-600 font-bold mb-4">{deptDeleteError}</p>}
            <div className="flex gap-3">
              <button onClick={handleDeptDeleteConfirmed} className="btn-danger flex-1">
                削除する
              </button>
              <button onClick={() => setDeptDeleteTarget(null)} className="btn-secondary flex-1">
                キャンセル
              </button>
            </div>
          </div>
